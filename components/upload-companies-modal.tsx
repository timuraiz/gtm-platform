...
import { uploadCompanies, type UploadedCompanyRow } from '@/app/actions/companies'

type FieldKey = 'domain' | 'name' | 'linkedin_url'

const FIELDS: { key: FieldKey; label: string; hints: string[]; required?: boolean }[] = [
  { key: 'domain', label: 'Domain', hints: ['domain', 'website', 'site', 'url', 'company_domain'], required: true },
  { key: 'name', label: 'Company name', hints: ['name', 'company', 'company_name', 'organization'] },
  { key: 'linkedin_url', label: 'LinkedIn URL', hints: ['linkedin', 'linkedin_url'] },
]
...
function autoDetect(headers: string[]): Record<FieldKey, number | null> {
  const map: Record<FieldKey, number | null> = { domain: null, name: null, linkedin_url: null }
...
export function UploadCompaniesModal({
  projectId,
  onClose,
}: {
  projectId: string
  ...
  const [result, setResult] = useState<{ inserted: number; attached: number; skipped: number } | null>(null)
  ...
  async function handleSubmit() {
    ...
    if (mapping.domain === null) { setError('Domain column is required'); return }
    setSubmitting(true)
    setError(null)
    try {
      const uploadRows: UploadedCompanyRow[] = []
      for (const row of parsed.rows) {
        ...
        if (!domain) continue
        uploadRows.push({ domain, name: get('name') ?? null, linkedin_url: get('linkedin_url') ?? null })
      }
      const res = await uploadCompanies(projectId, uploadRows)
      setResult(res)
      router.refresh()
    } catch (e) {
            ...
            {!parsed && (
              <label className="flex flex-col items-center justify-center gap-3 py-12 border-2 border-dashed border-zinc-200 rounded-2xl bg-zinc-50/50 hover:bg-zinc-50 hover:border-zinc-300 transition-colors cursor-pointer">
                ...
                <div className="text-center">
                  ...
                  <p className="text-xs text-zinc-400 mt-1">Required: domain · Optional: name, linkedin_url</p>
                </div>
                <input
                  type="file"
                  accept=".csv,text/csv"
            ...
            {result && (
              <div className="flex flex-col items-center justify-center py-10 gap-3">
                ...
                <p className="text-sm font-medium text-zinc-900">
                  {result.attached} compan{result.attached === 1 ? 'y' : 'ies'} attached
                </p>
                <p className="text-xs text-zinc-400">
                  {result.inserted} new · {result.attached - result.inserted} reused from team space
          ...
          {parsed && !result && (
            <div className="flex items-center justify-end gap-2 px-6 py-4 border-t border-zinc-100">
              ...
              <button
                ...
                disabled={submitting || mapping?.domain === null}
              ...
              >
                {submitting ? 'Importing…' : `Import ${parsed.rows.length} companies`}
              </button>
            </div>
          )}
          {result && (
            <div className="flex items-center justify-end px-6 py-4 border-t border-zinc-100">
              <button onClick={onClose} className="rounded-lg bg-zinc-900 text-white px-4 py-1.5 text-xs font-medium hover:bg-zinc-700 transition-colors">
                Done
              </button>
            </div>
          )}

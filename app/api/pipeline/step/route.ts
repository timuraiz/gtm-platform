...
async function runGenerateFilters(icp: unknown, targetSegment?: TargetSegment | null) {
  ...
  if (targetSegment?.geo) {
    filters.locations = [targetSegment.geo]
    // Auto-exclude the project's other geos so Apollo doesn't sneak in companies
    // that have HQs in those even if a keyword matches. e.g. project geo
    // ["CIS","Europe","USA","Asia"] + iteration geo "Europe" → exclude others.
    const projectGeos = ((apolloFilters.locations as string[] | undefined) ?? (icpData.geo as string[] | undefined) ?? []) as string[]
    const exclude = projectGeos.filter(g => g && g !== targetSegment.geo)
    if (exclude.length > 0) {
      const existing = (filters.not_locations as string[] | undefined) ?? []
      filters.not_locations = [...new Set([...existing, ...exclude])]
    }
  }
  return filters
}
...
async function runApolloSearch(filters: Record<string, unknown>, icp: Record<string, unknown>, page = 1, seenDomains: string[] = [], overrideKeywords?: string[]) {
  ...
  // New optional Apollo filters generated by the apollo-filter-mapping skill
  // (or set manually via run_config / icp.apollo_filters). Each one tightens
  // discovery so the classifier sees a smaller, more relevant batch.
  const notLocations = (filters.not_locations as string[] | undefined) ?? []
  const technologies = (filters.technologies as string[] | undefined) ?? []
  const jobTitles = (filters.job_titles as string[] | undefined) ?? []
  const minActiveJobs = filters.min_active_jobs as number | undefined
  const latestFundingDateMin = filters.latest_funding_date_min as string | undefined
  const latestFundingDateMax = filters.latest_funding_date_max as string | undefined
    ...
    try {
      const res = await fetch(`${APOLLO_BASE}/mixed_companies/search`, {
        ...
        body: JSON.stringify({
          ...
          // Tightening filters — only sent when present so we don't accidentally
          // require fields the user didn't ask for.
          ...(notLocations.length ? { organization_not_locations: notLocations } : {}),
          ...(technologies.length ? { currently_using_any_of_technology_uids: technologies } : {}),
          ...(jobTitles.length ? { q_organization_job_titles: jobTitles } : {}),
          ...(minActiveJobs ? { 'organization_num_jobs_range[min]': minActiveJobs } : {}),
          ...(latestFundingDateMin ? { 'latest_funding_date_range[min]': latestFundingDateMin } : {}),
          ...(latestFundingDateMax ? { 'latest_funding_date_range[max]': latestFundingDateMax } : {}),
          ...(revenueMin ? { 'revenue_range[min]': revenueMin } : {}),
          ...(revenueMax ? { 'revenue_range[max]': revenueMax } : {}),
          page, per_page: 50,
        }),
